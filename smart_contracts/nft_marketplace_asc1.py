...
class NFTMarketplaceASC1(NFTMarketplaceInterface):
    ...
    def app_initialization(self):
        ...
        return Seq([
            ...
            App.globalPut(self.Variables.asa_token, Txn.assets[1]),
    ...
    def initialize_escrow(self, escrow_address):
        ...
        asset_escrow = AssetParam.clawback(Int(0))
    ...
    def make_sell_offer(self, sell_price):
        ...
        return If(can_sell,update_state,Return(Int(0)))

    def buy(self):
        """
        Atomic transfer of 4 transactions:
        1. Application call.
        2. Turn back token from buyer to dev.
        3. Payment from dev to seller.
        4. Asset transfer from escrow to buyer.
        :return:
        """
        ...
        valid_payment_from_buyer_to_dev = And(
            Gtxn[1].type_enum() == TxnType.AssetTransfer,
            Gtxn[1].asset_amount() == App.globalGet(self.Variables.asa_price),
            Gtxn[1].xfer_asset() == App.globalGet(self.Variables.asa_token),
        )

        valid_payment_to_seller = And(
            Gtxn[2].type_enum() == TxnType.AssetTransfer,
            Gtxn[2].asset_amount() == App.globalGet(self.Variables.asa_price),
            Gtxn[2].xfer_asset() == App.globalGet(self.Variables.asa_token),
        )

        valid_asa_transfer_from_escrow_to_buyer = And(
            Gtxn[3].type_enum() == TxnType.AssetTransfer,
            # Gtxn[3].sender() == App.globalGet(self.Variables.escrow_address),
            Gtxn[3].xfer_asset() == App.globalGet(self.Variables.asa_id),
            Gtxn[3].asset_amount() == Int(1)
        )

        can_buy = And(valid_number_of_transactions,
                      asa_is_on_sale,
                      valid_payment_from_buyer_to_dev,
                      valid_payment_to_seller,
                      valid_asa_transfer_from_escrow_to_buyer)

        update_state = Seq([
            App.globalPut(self.Variables.asa_owner, Gtxn[1].sender()),
            App.globalPut(self.Variables.app_state, self.AppState.active),
            Return(Int(1))
        ])

        return If(can_buy,update_state,Return(Int(0)))
    ...
    def stop_sell_offer(self):
        ...
        return If(can_stop_selling,update_state,Return(Int(0)))
